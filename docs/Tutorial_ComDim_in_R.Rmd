...
```{r quick, echo = TRUE, message = FALSE}
  ...
  # Select the important features in the rb from b2 (so, blocks 2-4).
...
To generate the RBs, we can simply use the __SplitRB()__ function with a multi-block object that contains the *batches* information, which must be a *numeric vector*.
...
In the previous example, __mb__ was composed of 1 block only. Of course, __SplitRB()__ can be used with a real multi-block contaning more than 1 block.
...
ComDim is executed using the _comDim_PCA()_ function. This function accepts as inputs the multi-block data sets generated with _SplitRB()_ and _BuildMultiBlock()_ (as long as their number of samples are conserved for all the blocks).
...
As in other matrix decomposition methods, we can find the most descriptive variables for each component.
In this tutorial we propose to select, for a given component, those **variables presenting the highest correlations and covariances** as calculated from the corresponding vector of __Global scores__ and the corresponding __replicate block__ used in __ComDim_PCA()__. Then, in order to provide an unique list of significant variables, only those **common to all replicate blocks** and presenting the **same loadings sign** were regarded as significant variables.

In other words, continuing with the miRNAseq data from the previous example, variables will be considered to be **truly** significant only if they were significant in __all the 3 blocks of RB data__ and presented the __same evolution__ (positive or negative) over the course of the experiment. This can be calculated with the function __SelectFeaturesRB()__ using the code below.

```{r variable_selection, echo=TRUE, error = TRUE}
features <- SelectFeaturesRB(RB = rb_d3, results = results,
