...
###Function to create a multi-block structure suitable for ComDim_PCA_2020.R

###INPUT DATA:
#  newBlock: the data to be incorporated into the multi-block structure.
#  newVars: the variable names (optional)
#  newSamples: the sample names. The sample replicates should have the same name (optional).
#  growingMB: The name of the multi-block structure it this already exists (optional).
#  batches: A vector indicating the batch number for every sample (optional).
#  ...: Additional objects descriptive of newBlock can be introduced, as long as they have the same length as the sample size.
